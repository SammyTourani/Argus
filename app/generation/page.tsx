...
import RuntimeStatus, { RuntimeStatusType } from '@/components/app/generation/RuntimeStatus';
...
function AISandboxPage() {
  ...
  const [runtimeStatus, setRuntimeStatus] = useState<RuntimeStatusType>('idle');
    ...
    try {
      ...
      if (generatedCode) {
        ...
        if (activeSandboxData && generatedCode) {
          ...
          // ---------------------------------------------------------
          // Phase 2: Runtime Monitoring & Auto-Fix Loop
          // ---------------------------------------------------------
          if (appConfig.runtimeMonitoring.enabled && activeSandboxData) {
            console.log('[AutoFix] Starting monitoring loop...');
            setRuntimeStatus('monitoring');

            let attempts = 0;
            const maxRetries = appConfig.runtimeMonitoring.maxRetries;
            let hasErrors = true;

            while (hasErrors && attempts < maxRetries) {
              // 1. Monitor Runtime
              addChatMessage(`Checking for runtime errors...`, 'system');

              try {
                const monitorResponse = await fetch('/api/monitor-runtime', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({
                    sandboxUrl: activeSandboxData.url,
                ...
                if (monitorResult.success && monitorResult.hasErrors) {
                  // Errors detected!
                  const count = monitorResult.errors.length;
                  setErrorCount(count);

                  if (!autoFixEnabled) {
                    setRuntimeStatus('error');
                    addChatMessage(`⚠️ Detected ${count} runtime error${count > 1 ? 's' : ''}. Auto-fix is disabled.`, 'system');
                    break; // Stop loop if auto-fix is disabled
                  }

                  setRuntimeStatus('fixing');
                  addChatMessage(
                    `⚠️ Detected ${count} runtime error${count > 1 ? 's' : ''}. Auto-fixing (Attempt ${attempts + 1}/${maxRetries})...`,
                    'system'
                  );

                  // 2. Request Auto-Fix
                  const fixResponse = await fetch('/api/fix-runtime-errors', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                      sandboxId: activeSandboxData.sandboxId,
                  ...
                  if (!fixResponse.ok) {
                    throw new Error(`Auto-fix request failed: ${fixResponse.statusText}`);
                  }

                  // 3. Consume Stream & Reconstruct Code
                  ...
                  if (reader) {
                    ...
                    while (true) {
                      const { done, value } = await reader.read();
                      if (done) break;
                      fixedCode += decoder.decode(value, { stream: true });
                    }
                  }

                  if (fixedCode && fixedCode.trim().length > 0) {
                    // 4. Apply Fix
                    console.log('[AutoFix] Applying fix...');
                    await applyGeneratedCode(fixedCode, true, activeSandboxData);
                    attempts++;

                    // Add a small delay before re-monitoring to allow app to reload
                    await new Promise(resolve => setTimeout(resolve, 3000));
                  } else {
                    console.warn('[AutoFix] No fix code generated');
                    break;
                  }

                } else {
                  // No errors!
                  console.log('[AutoFix] No runtime errors detected.');
                  hasErrors = false;
                  setErrorCount(0);
                  setRuntimeStatus('success');

                  if (attempts > 0) {
                    addChatMessage('✅ Auto-fix successful! Runtime errors resolved.', 'system');
                  } else {
                    addChatMessage('✅ No runtime errors detected.', 'system');
                  }

                  // Reset to idle after a delay
                  setTimeout(() => setRuntimeStatus('idle'), 5000);
                }
              } catch (monitorError) {
                console.error('[AutoFix] Error in loop:', monitorError);
                setRuntimeStatus('error');
                // Don't block the flow if monitoring fails
                break;
              }
            }
            ...
            if (hasErrors && attempts >= maxRetries) {
              setRuntimeStatus('error');
              addChatMessage('⚠️ Auto-fix limit reached. Some errors may persist.', 'system');
            }
          }
          // ---------------------------------------------------------
          ...
          <div className="flex-1 max-w-[400px] flex flex-col border-r border-border bg-background">
            ...
            <RuntimeStatus
              ...
              onToggleAutoFix={() => {
                ...
                console.log(`[UX] User toggled Auto-Fix: ${newState ? 'ON' : 'OFF'}`);
