...
function SignInContent() {
  ...
  return (
    <AuthLayout mode="signin">
      ...
      {/* Auth mode toggle */}
      <div className="flex rounded-lg overflow-hidden mb-6 border" style={{ borderColor: 'rgba(0,0,0,0.1)' }}>
        <button
          type="button"
          onClick={() => { setAuthMode('password'); setMagicLinkSent(false); }}
          className="flex-1 py-2 text-xs font-medium transition-colors"
          style={{
            background: authMode === 'password' ? '#FA4500' : 'transparent',
            color: authMode === 'password' ? 'white' : '#888',
          }}
        >
          Password
        </button>
        <button
          ...
          className="flex-1 py-2 text-xs font-medium transition-colors"
          style={{
            background: authMode === 'magiclink' ? '#FA4500' : 'transparent',
            color: authMode === 'magiclink' ? 'white' : '#888',
          }}
        >
          Magic Link
        </button>
      ...
      {magicLinkSent && authMode === 'magiclink' ? (
        ...
        <form onSubmit={handleEmailSignIn} className="flex flex-col gap-4">
          ...
          <div className="text-right">
            <Link href="/forgot-password" className="text-xs hover:underline" style={{ color: '#FA4500' }}>
              Forgot password?
            </Link>
          </div>
        </form>
      ...
      <p className="text-center text-xs mt-8" style={{ color: '#888' }}>
        New to Argus?{' '}
        <Link href="/sign-up" className="font-medium hover:underline" style={{ color: '#FA4500' }}>
          Sign up →
        </Link>
      </p>
    </AuthLayout>
