...
import { Save, Key, Bell, CreditCard, User, ExternalLink, Zap } from 'lucide-react';
    ...
    <DashboardShell>
      <div className="max-w-4xl mx-auto px-6 py-8 flex flex-col md:flex-row gap-8">
        <aside className="md:w-48 shrink-0">
          <nav className="flex md:flex-col gap-1 overflow-x-auto md:overflow-visible md:sticky md:top-20 pb-2 md:pb-0 border-b md:border-b-0 border-zinc-200">
            {SECTIONS.map(s => {
              const Icon = s.icon;
              return (
                <button key={s.id} onClick={() => setActiveSection(s.id)} className={cn(
                  ...
                  activeSection === s.id ? 'bg-orange-50 text-[#FA4500]' : 'text-zinc-600 hover:bg-zinc-100 hover:text-zinc-900'
                )}>
              ...
              <div className="rounded-xl border border-zinc-200 bg-white p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input type="text" value={fullName} onChange={e => setFullName(e.target.value)} className="w-full rounded-lg border border-zinc-200 px-3 py-2.5 text-sm focus:border-[#FA4500] focus:outline-none transition-colors" />
                  </div>
                  <div>
            ...
            {activeSection === 'model' && (
              <div className="rounded-xl border border-zinc-200 bg-white p-6">
                ...
                <div className="space-y-2 mb-6">
                  {MODELS.map(m => (
                    <label key={m.id} className={cn('flex items-center gap-3 rounded-lg border p-3 cursor-pointer transition-colors', defaultModel === m.id ? 'border-[#FA4500] bg-orange-50' : 'border-zinc-200 hover:border-zinc-300')}>
                      <input type="radio" name="model" value={m.id} checked={defaultModel === m.id} onChange={() => setDefaultModel(m.id)} className="accent-[#FA4500]" />
                      <div>
                ...
                <button onClick={handleSaveModel} disabled={saving} className="flex items-center gap-2 rounded-lg bg-[#FA4500] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#e03e00] disabled:opacity-60 transition-colors">
                  <Save size={14} />{saving ? 'Saving...' : saved ? '✓ Saved' : 'Save preference'}
                </button>
              </div>
            )}
            ...
            {activeSection === 'billing' && (
              <div className="space-y-6">
                <div className="rounded-xl border border-zinc-200 bg-white p-6">
                  ...
                  <div className={cn('rounded-lg border p-5 mb-6', subscription.tier === 'free' ? 'bg-zinc-50 border-zinc-200' : 'bg-orange-50 border-[#FA4500]/20')}>
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-semibold text-zinc-900 capitalize">{subscription.tier} Plan</p>
                          {subscription.tier !== 'free' && <span className="rounded-full bg-[#FA4500] px-2.5 py-0.5 text-[10px] font-bold text-white uppercase tracking-wider">Active</span>}
                        </div>
                        <p className="text-xs text-zinc-400 mt-1">
                          {subscription.tier === 'free' && '$0/month'}{subscription.tier === 'pro' && '$19/month'}{subscription.tier === 'team' && '$49/month'}{subscription.tier === 'enterprise' && 'Custom pricing'}
                        </p>
                      </div>
                      <span className={cn('rounded-full px-2.5 py-1 text-xs font-medium', subscription.tier === 'free' ? 'bg-zinc-200 text-zinc-600' : 'bg-[#FA4500]/10 text-[#FA4500]')}>Current</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    {subscription.tier !== 'free' && (
                      <button onClick={handleManageBilling} disabled={managingBilling} className="flex items-center gap-2 rounded-lg border border-zinc-200 px-4 py-2.5 text-sm font-medium text-zinc-700 hover:bg-zinc-50 disabled:opacity-60 transition-colors">
                        <ExternalLink size={14} />{managingBilling ? 'Opening...' : 'Manage Subscription'}
                      </button>
                    )}
                    {subscription.tier !== 'free' && (
                      <button onClick={handleManageBilling} disabled={managingBilling} className="text-sm text-zinc-500 hover:text-zinc-700 transition-colors">View invoices</button>
                    )}
                  </div>
                </div>
                {subscription.tier === 'free' && (
                      ...
                      {[
                        ...
                        <div key={plan.name} className="flex items-center justify-between rounded-lg border border-zinc-200 p-4 hover:border-zinc-300 transition-colors">
                          ...
                          <div className="text-right shrink-0 ml-4">
                            ...
                            <button onClick={() => handleUpgrade(plan.planKey)} disabled={upgradingPlan === plan.planKey} className="mt-1 text-xs font-semibold text-[#FA4500] hover:underline disabled:opacity-60">
                ...
                <div className="rounded-xl border border-zinc-200 bg-white p-6">
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex items-center justify-between py-2 border-b border-zinc-100"><span className="text-zinc-500">Email</span><span className="text-zinc-900 font-medium">{email}</span></div>
                    <div className="flex items-center justify-between py-2 border-b border-zinc-100"><span className="text-zinc-500">Name</span><span className="text-zinc-900 font-medium">{fullName || 'Not set'}</span></div>
                    <div className="flex items-center justify-between py-2"><span className="text-zinc-500">Plan</span><span className="text-zinc-900 font-medium capitalize">{subscription.tier}</span></div>
                  </div>
                </div>
              </div>
            )}
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={item.label} className="flex items-center justify-between py-3 border-b border-zinc-100 last:border-0">
                      <div><p className="text-sm font-medium text-zinc-900">{item.label}</p><p className="text-xs text-zinc-400 mt-0.5">{item.desc}</p></div>
                      <button onClick={() => item.set(!item.state)} className={cn('relative h-6 w-11 rounded-full transition-colors duration-200', item.state ? 'bg-[#FA4500]' : 'bg-zinc-200')}>
                        <span className={cn('absolute top-0.5 h-5 w-5 rounded-full bg-white shadow-sm transition-transform duration-200', item.state ? 'translate-x-5' : 'translate-x-0.5')} />
                      </button>
                    </div>
                  ))}
                </div>
                <button onClick={() => { try { localStorage.setItem('argus_notification_prefs', JSON.stringify({ notifyBuilds, notifyInvites, notifyMarketing })); } catch {} setSaved(true); setTimeout(() => setSaved(false), 2000); }} className="mt-6 flex items-center gap-2 rounded-lg bg-[#FA4500] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#e03e00] transition-colors">
