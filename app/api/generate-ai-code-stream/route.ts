...
// Provider management is centralized in lib/ai/provider-manager.ts
// It handles AI Gateway, caching, and all 4 providers (OpenAI, Anthropic, Groq, Google)
...
It's better to have 3 complete files than 10 incomplete files.`
        ...
        try {
          const fallbackResult = await streamWithFallback(
            {
              primary: model,
              fallbacks: DEFAULT_FALLBACK_CHAIN.fallbacks.filter(m => m !== model),
              maxRetries: 2,
              retryDelayMs: 1000,
            },
            async (fallbackModelId: string) => {
              ...
              if (fallbackModelId.startsWith('openai/gpt-5')) {
                delete streamOptions.temperature;
              } else if (!streamOptions.temperature) {
                streamOptions.temperature = 0.7;
              }

              result = await streamText(streamOptions);
              // Return a stream to satisfy the fallback chain's contract
              // We use result.textStream separately for parsing, this is just for the chain
              return result.textStream as unknown as ReadableStream;
            },
            (fromModel, toModel, error) => {
              console.log(`[generate-ai-code-stream] Falling back: ${fromModel} → ${toModel} (${error.message})`);
          ...
          await sendProgress({
            type: 'error',
            message: `All models failed. Last error: ${finalError.message}`
          });
          throw finalError;
        }
...
Provide the complete file content without any truncation. Include all necessary imports, complete all functions, and close all tags properly.`;
    ...
    return new Response(stream.readable, {
      headers: {
        ...
        'Access-Control-Allow-Origin': process.env.NEXT_PUBLIC_APP_URL || 'https://buildargus.dev',
