...
export async function sendWelcomeEmail(to: string, name?: string) {
  ...
  const html = baseLayout(`
    ...
    <p>You're on the <span class="highlight">Free plan</span> — 30 credits per month included. No credit card needed.</p>
...
export async function sendFirstBuildPrompt(to: string, name?: string) {
  ...
  const html = baseLayout(`
    <h1>Your 30 free credits are waiting, ${firstName}</h1>
    ...
    <p style="color: rgba(255,255,255,0.4); font-size: 13px;">You have 30 free credits this month. No credit card required.</p>
  `)

  return getResend().emails.send({
    from: FROM,
    to,
    subject: 'Your 30 free credits are waiting',
    html,
  })
}
...
export async function sendUpgradePrompt(to: string, buildsUsed: number) {
  const html = baseLayout(`
    <h1>Your Argus credits are running low 🚀</h1>
    ...
    <ul style="color: rgba(255,255,255,0.6); font-size: 14px; padding-left: 20px; margin: 16px 0;">
      <li style="margin-bottom: 8px;"><strong style="color: white">300 credits/month</strong> — 10x more than Free</li>
      <li style="margin-bottom: 8px;"><strong style="color: white">Priority sandbox</strong> — faster generation</li>
      <li style="margin-bottom: 8px;"><strong style="color: white">All 9 AI models</strong> — Claude, GPT-4o, Gemini, and more</li>
      <li style="margin-bottom: 8px;"><strong style="color: white">Unlimited free models</strong> — keep building after credits</li>
    </ul>
    <a href="${SITE_URL}/api/stripe/create-checkout-session" class="btn">Upgrade to Pro — $19/month</a>
    <p style="color: rgba(255,255,255,0.4); font-size: 13px;">Or wait until the 1st for your credits to reset.</p>
  `)

  return getResend().emails.send({
    from: FROM,
    to,
    subject: 'Your Argus credits are running low',
    html,
  })
}
...
export async function sendSubscriptionConfirmed(to: string, name?: string) {
  const html = baseLayout(`
    ...
    <ul style="color: rgba(255,255,255,0.6); font-size: 14px; padding-left: 20px; margin: 16px 0;">
      <li style="margin-bottom: 6px;">✓ 300 credits per month</li>
...
export async function sendSubscriptionCanceled(to: string, endsAt?: string) {
  const html = baseLayout(`
    ...
    <p>Your Pro access ends <strong>${endsAt || 'at the end of your billing period'}</strong>. After that, you'll be on the Free plan (30 credits/month).</p>
